...
def main():
    ...
    canvas = tuple([int(c) for c in canvas.split()])

    canvas_color = input('Now enter if the canvas should be white (W) or black (B): W/B\n>> ')
    canvas = Canvas(canvas, WHITE if canvas_color.upper() == 'W' else BLACK)

    print('Now let\'s create some colorful shapes!')

    while True:
        while True:
            shape = input('Do you want to paint a square (S) or a rectangle (R), only S or R are valid inputs:\n>> ')
        ...
        if shape.upper() == 'S':
            shape_to_paint = shapes.Square(x, y, width, color)
        else:
            shape_to_paint = shapes.Rectangle(x, y, width, height, color)
